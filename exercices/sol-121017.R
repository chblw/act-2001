...
neglogvrais <- function(params) {
  deltaLambda <- params[1] + params[2] * (2 * (seq_along(count_data) - 1) + 1) / 2
  - sum(count_data * log(deltaLambda) - deltaLambda - log(factorial(count_data))) 
}
...
mle_params$par[1] + mle_params$par[2] / 2 * (2 * c(100, 110) - 1)
